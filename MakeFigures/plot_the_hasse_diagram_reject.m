function[] = plot_the_hasse_diagram_reject(G, nodes_info, D, iter_num, to_save)
% plotting the graph according to D
if nargin<5
    to_save = 0;
end

num_subgroups = size(G.Nodes,1);  % number of nodes
d = nodes_info(1).order;

if length(D) ~= num_subgroups
    error('Length of D (%d) must match number of nodes (%d)', length(D), num_subgroups);
end
...
title(sprintf('DAG Sequential Rejection, iteration %d', iter_num ...
    ));
