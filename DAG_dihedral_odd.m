function [G, nodes_info] = DAG_dihedral_odd(n)
...
%
    ...
    for i = 1:num_subgroups
        ...
        % adding cyclic node
        if and(m~=n,m~=1)
            nodes_info(i+num_subgroups-1).order = m;
        ...
        for j = i+1:num_subgroups
            if mod(divisors(i), divisors(j)) == 0
                ...
                if is_minimal
                    ...
                    if (j + num_subgroups-1)<=size(nodes_info,2)
                        edges = [edges; j (j + num_subgroups-1)]; % from Dm to Cm
                    end
                    %inside Cm, m<d
                    if and(i>1,(j + num_subgroups-1)<=size(nodes_info,2))
                        edges = [edges; (i + num_subgroups-1) (j + num_subgroups-1)];
                    end
                end
            end
        end
